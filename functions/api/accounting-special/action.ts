...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    if (action === 'save-book-type') {
      ...
      const system = await env.DB.prepare(`SELECT system_type FROM accounting_book_types WHERE code=?`)
        .bind(code).first<{ system_type: number }>();
      if (system?.system_type) return json({ ok: false, message: '기본 회계구분은 변경할 수 없습니다.' }, 400);
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_book_types
        ...
        audit(env.DB, 'save', 'book-type', code, me.id, me.name, { code, name }, now),
      ]);
    ...
    if (action === 'save-entity') {
      ...
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_entities
        ...
        audit(env.DB, 'save', 'entity', id, me.id, me.name, { entityCode, name, entityType }, now),
      ]);
    ...
    if (action === 'save-fund') {
      ...
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_funds
        ...
        audit(env.DB, 'save', 'fund', id, me.id, me.name, { fundCode, name, fundType }, now),
      ]);
    ...
    if (action === 'save-donor') {
      ...
      const existing = await env.DB.prepare(`SELECT donor_no FROM accounting_donors WHERE id=?`)
        .bind(id).first<{ donor_no: string }>();
      const donorNo = existing?.donor_no || await nextSpecialNumber(env.DB, 'donor');
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_donors
        ...
        audit(env.DB, 'save', 'donor', id, me.id, me.name, { donorNo, donorType, name }, now),
      ]);
    ...
    if (action === 'save-donation') {
      ...
      if (donorId) {
        const donor = await env.DB.prepare(`SELECT id FROM accounting_donors WHERE id=? AND active=1`).bind(donorId).first();
      ...
      const accounts = await env.DB.prepare(`SELECT code,account_type FROM accounting_accounts WHERE code IN (?,?) AND active=1`)
      ...
      const donationNo = await nextSpecialNumber(env.DB, 'donation', year);
      const receiptRequested = payload.receiptRequested === true ? 1 : 0;
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_donations
        ...
        audit(env.DB, 'create', 'donation', id, me.id, me.name, { donationNo, amount, donorId, ...dimensions }, now),
      ]);
      let posting: any = null;
      if (payload.postNow === true && manager) {
        const donation = await env.DB.prepare(`SELECT d.*,COALESCE(o.name,'익명') AS donor_name,e.department_path AS department
        ...
        posting = await postDonation(env.DB, donation, me.name);
      }
    ...
    if (action === 'post-donation') {
      ...
      const posting = await postDonation(env.DB, donation, me.name);
      await audit(env.DB, 'post', 'donation', id, me.id, me.name, posting, now).run();
    ...
    if (action === 'issue-receipt' || action === 'cancel-receipt') {
      const id = clean(payload.id, 80);
      const donation = await env.DB.prepare(`SELECT d.*,o.name AS donor_name FROM accounting_donations d
      ...
      if (action === 'issue-receipt') {
        const receiptNo = donation.receipt_no || await nextSpecialNumber(env.DB, 'receipt', Number(donation.fiscal_year));
        await env.DB.batch([
          env.DB.prepare(`UPDATE accounting_donations
          ...
          audit(env.DB, 'issue', 'donation-receipt', id, me.id, me.name, { receiptNo }, now),
        ]);
        return json({ ok: true, receiptNo, message: '기부금영수증 발급내역을 등록했습니다.' });
      }
      await env.DB.batch([
        env.DB.prepare(`UPDATE accounting_donations SET receipt_status='cancelled',receipt_cancelled_at=?,updated_at=? WHERE id=?`)
          .bind(now, now, id),
        audit(env.DB, 'cancel', 'donation-receipt', id, me.id, me.name, { receiptNo: donation.receipt_no }, now),
      ]);
    ...
    if (action === 'save-asset') {
      ...
      const existing = await env.DB.prepare(`SELECT asset_no FROM accounting_assets WHERE id=?`)
      ...
      const dimensions = await validateDimensions(env.DB, payload);
      const assetNo = existing?.asset_no || await nextSpecialNumber(env.DB, 'asset', Number(date.slice(0, 4)));
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_assets
        ...
        audit(env.DB, 'save', 'asset', id, me.id, me.name, { assetNo, name, cost }, now),
      ]);
    ...
    if (action === 'dispose-asset') {
      ...
      await env.DB.batch([
        env.DB.prepare(`UPDATE accounting_assets
        ...
        audit(env.DB, 'dispose', 'asset', id, me.id, me.name, payload, now),
      ]);
    ...
    if (action === 'save-card') {
      ...
      const existing = await env.DB.prepare(`SELECT card_code FROM accounting_cards WHERE id=?`)
      ...
      const dimensions = await validateDimensions(env.DB, payload);
      const cardCode = existing?.card_code || await nextSpecialNumber(env.DB, 'card');
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_cards
        ...
        audit(env.DB, 'save', 'card', id, me.id, me.name, { cardCode, label }, now),
      ]);
    ...
    if (action === 'save-card-transaction') {
      ...
      const card = await env.DB.prepare(`SELECT * FROM accounting_cards WHERE id=? AND active=1`).bind(cardId).first<any>();
      ...
      const dimensions = await validateDimensions(env.DB, {
      ...
      const txNo = await nextSpecialNumber(env.DB, 'card-tx', Number(date.slice(0, 4)));
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_card_transactions
        ...
        audit(env.DB, 'create', 'card-transaction', id, me.id, me.name, { txNo, amount, merchant }, now),
      ]);
    ...
    if (action === 'post-card-transaction') {
      ...
      const id = clean(payload.id, 80);
      const tx = await env.DB.prepare(`SELECT t.*,c.settlement_account_code FROM accounting_card_transactions t
      ...
      await env.DB.batch([
        ...
        env.DB.prepare(`UPDATE accounting_card_transactions SET journal_id=?,status='posted',updated_at=? WHERE id=?`)
          .bind(journalId, now, id),
        audit(env.DB, 'post', 'card-transaction', id, me.id, me.name, { journalNo }, now),
      ]);
    ...
    if (action === 'save-branch-report') {
      ...
      await env.DB.batch([
        env.DB.prepare(`INSERT INTO accounting_branch_reports
        ...
        audit(env.DB, status === 'submitted' ? 'submit' : 'save', 'branch-report', id, me.id, me.name,
    ...
    if (action === 'review-branch-report') {
      ...
      await env.DB.batch([
        env.DB.prepare(`UPDATE accounting_branch_reports
        ...
        audit(env.DB, decision, 'branch-report', id, me.id, me.name, { memo: clean(payload.memo, 1000) }, now),
      ]);
