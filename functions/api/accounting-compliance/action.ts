import { authenticateSession, clean, ensureTables, json, normalizeDepartmentValue, randomHex } from '../../_shared/helpers';
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    ...
    if (action === 'save-procurement') {
      ...
      const responsibleUserId = clean(payload.responsibleUserId, 100);
      let responsibleName = '';
      if (responsibleUserId) {
        const responsible = await env.DB.prepare(`SELECT name FROM system_users WHERE CAST(id AS TEXT)=? AND active=1`).bind(responsibleUserId).first<{ name: string }>();
        if (!responsible?.name) return json({ ok: false, message: '선택한 책임자(담당자) 계정을 찾을 수 없습니다.' }, 400);
        responsibleName = clean(responsible.name, 100);
      }
      await db.batch([
