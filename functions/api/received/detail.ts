...
export const onRequestPost:PagesFunction<Env>=async({request,env})=>{
  ...
  const id=clean(payload.id,60);const document=await env.DB.prepare(`SELECT id,direction,title,counterparty,source_system,external_doc_number,memo,department,related_document_id,handled_by,CAST(handled_by_user_id AS TEXT) AS handled_by_user_id,received_at,created_at,updated_at FROM received_documents WHERE id=?`).bind(id).first();
