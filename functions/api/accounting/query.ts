  ...
  try{
    if(action==='init'){
      const [fiscal,accounts,summaryRows,recentRows,closingRows,dimensions,integration]=await Promise.all([
        ...
        db.prepare(`SELECT id,period_month,status,closed_by,closed_at,memo FROM accounting_closings WHERE fiscal_year=? ORDER BY period_month`).bind(year).all(),
    ...
    if(action==='budgets'||action==='budget-execution'||action==='budgets-export'){
      ...
      if(department){if(department==='위원회·신도단체 - 진향회(향천사)'){conditions.push('(b.department=? OR b.department=? OR b.department=? OR b.department=?)');values.push(department,'진향회(향천사)','위원회·신도단체 - 신도회','신도회')}else if(department==='총무원 - 교무부'){conditions.push('(b.department=? OR b.department=? OR b.department=?)');values.push(department,'총무원 - 교육부','교육부')}else{conditions.push('b.department=?');values.push(department)}}if(project){conditions.push('b.project=?');values.push(project)}if(bookTypeCode){conditions.push('b.book_type_code=?');values.push(bookTypeCode)}if(entityId){conditions.push('b.entity_id=?');values.push(entityId)}if(fundId){conditions.push('b.fund_id=?');values.push(fundId)}
