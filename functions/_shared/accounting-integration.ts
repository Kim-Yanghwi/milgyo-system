...
const applyCreate=async(db:D1Database,payload:any,eventId:string)=>{
  ...
  await db.batch([
    db.prepare(`INSERT INTO accounting_resolutions (id,resolution_no,resolution_type,fiscal_year,resolution_date,title,department,project,counterparty,account_code,settlement_account_code,amount,tax_amount,payment_method,memo,vendor_id,contract_id,budget_exception_id,document_id,status,created_by_user_id,created_by_name,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET resolution_no=excluded.resolution_no,resolution_type=excluded.resolution_type,fiscal_year=excluded.fiscal_year,resolution_date=excluded.resolution_date,title=excluded.title,department=excluded.department,project=excluded.project,counterparty=excluded.counterparty,account_code=excluded.account_code,settlement_account_code=excluded.settlement_account_code,amount=excluded.amount,tax_amount=excluded.tax_amount,payment_method=excluded.payment_method,memo=excluded.memo,vendor_id=COALESCE(excluded.vendor_id,accounting_resolutions.vendor_id),contract_id=COALESCE(excluded.contract_id,accounting_resolutions.contract_id),budget_exception_id=COALESCE(excluded.budget_exception_id,accounting_resolutions.budget_exception_id),document_id=excluded.document_id,status=CASE WHEN accounting_resolutions.status IN ('posted','cancelled') THEN accounting_resolutions.status ELSE excluded.status END,updated_at=excluded.updated_at`)
      .bind(r.id,r.resolutionNo,r.resolutionType,r.fiscalYear,r.resolutionDate,r.title,r.department||'',r.project||'',r.counterparty||'',r.accountCode,r.settlementAccountCode,Number(r.amount||0),Number(r.taxAmount||0),r.paymentMethod||null,r.memo||null,r.vendorId||null,r.contractId||null,r.budgetExceptionId||null,r.documentId,r.status||'approval_pending',r.createdByUserId,r.createdByName,r.createdAt||now,now),
