...
export const getTestResetPreview = async (env: ResetEnv) => {
  const [mainCounts, accountingCounts, mainFiles, accountingFiles] = await Promise.all([
    env.DB.prepare(`
      SELECT
        ...
        (SELECT COUNT(*) FROM foreign_application_forms WHERE form_type='거주숙소제공확인서') AS foreign_residence_forms,
        (SELECT COUNT(*) FROM foreign_application_forms WHERE form_type='통합신청서신고서') AS foreign_integrated_forms,
        (SELECT COUNT(*) FROM foreign_application_forms WHERE form_type='사증발급인정신청서') AS foreign_visa_eligibility_forms,
  ...
  return {
    ...
    main: {
      ...
      foreignResidenceForms: n(main.foreign_residence_forms),
      foreignIntegratedForms: n(main.foreign_integrated_forms),
      foreignVisaEligibilityForms: n(main.foreign_visa_eligibility_forms),
